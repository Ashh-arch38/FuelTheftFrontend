...
interface FuelUsageStats {
  totalFuelConsumed?: NumLike;
  totalFuelStolen?: NumLike;
  totalFuelRefueled?: NumLike;
  distanceTravelled?: NumLike;
  fuelEfficiency?: NumLike;
}
...
const FuelStatsGrid: React.FC<Props> = ({ stats, loading }) => {
  const consumed = fmt(stats?.totalFuelConsumed, "L");
  const stolen = fmt(stats?.totalFuelStolen, "L");
  const refueled = fmt(stats?.totalFuelRefueled, "L");
  const distance = fmt(stats?.distanceTravelled, "km");
  const efficiency = fmt(stats?.fuelEfficiency, "km/L");
