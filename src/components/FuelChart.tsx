...
interface ParsedDataPoint {
  ...
  fuelChange?: number;
  description?: string;
  sensorStatus?: "ONLINE" | "OFFLINE" | "FAULTY" | "WARNING"; // NEW: sensor status
  lastSeen?: string; // NEW: when sensor was last online
}
...
const DotRenderer: React.FC<any> = ({ cx, cy, payload }) => {
  ...
  const sensorStatus = payload.sensorStatus ?? "ONLINE";
  
  // If sensor is offline/faulty, show gray regardless of event
  let fill: string;
  if (sensorStatus === "OFFLINE" || sensorStatus === "FAULTY") {
    fill = "#6b7280"; // Gray for offline sensors
  } else {
    // Normal event colors for online sensors
    fill =
      event === "THEFT"
        ? "#ef4444"
        : event === "REFUEL"
  ...
  return <circle cx={cx} cy={cy} r={4} fill={fill} stroke="#fff" strokeWidth={1} className="transition-all duration-200" />;
};

/** Active (hover) dot renderer - preserves original event colors and sensor status */
const ActiveDotRenderer: React.FC<any> = ({ cx, cy, payload }) => {
  ...
  return <circle cx={cx} cy={cy} r={6} fill={fill} stroke="#000" strokeWidth={2} />;
};
          ...
          <Tooltip
            ...
            formatter={(value: number, _name: string, props: any) => {
              ...
              const fuelChange = props?.payload?.fuelChange as
                | number
                | undefined;
              const sensorStatus = props?.payload?.sensorStatus || "ONLINE";
              const lastSeen = props?.payload?.lastSeen;

              // Build the main event description
              ...
              if (event !== "NORMAL") {
                ...
                const changeStr =
                  typeof fuelChange === "number" && !Number.isNaN(fuelChange)
                    ...
                    : "";
                eventDescription = `${eventLabel}${changeStr}: ${
                  description || "Detected Event"
                }`;
              } else {
                eventDescription = "Fuel Level";
              }

              // Add sensor status information
              let sensorInfo = "";
              if (sensorStatus === "OFFLINE" || sensorStatus === "FAULTY") {
                sensorInfo = ` | Sensor: ${sensorStatus}`;
                if (lastSeen) {
                  try {
                    ...
                    if (hoursAgo > 0) {
                      sensorInfo += ` | Last Online: ${hoursAgo}h ago`;
                    } else {
                      sensorInfo += ` | Last Online: ${minutesAgo}m ago`;
                    }
                  } catch {
                    sensorInfo += ` | Last Online: Unknown`;
                  }
                }
              }

              return [`${value} L`, `${eventDescription}${sensorInfo}`];
            }}
            contentStyle={{
