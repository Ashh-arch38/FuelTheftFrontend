          ...
          <div className="relative">
            ...
            <div className="relative">
              <input
                type="text"
                value={searchTerm}
                placeholder="Click to see all buses or type to search..."
                onChange={handleSearchChange}
                onFocus={() => setShowSuggestions(true)}
                onClick={() => setShowSuggestions(true)}
                onBlur={() => setTimeout(() => setShowSuggestions(false), 200)}
                className="border pl-10 pr-3 py-2 rounded shadow-sm w-full dark:bg-gray-900 dark:border-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              />
            ...
            {showSuggestions && (
              <ul className="absolute z-10 mt-1 w-full bg-white dark:bg-gray-900 border dark:border-gray-700 rounded-lg shadow-lg max-h-60 overflow-auto">
                {/* Show "All Buses" option when no search term */}
                {!searchTerm && (
                  <li className="px-4 py-3 bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-600">
                    <div className="text-center text-sm text-gray-600 dark:text-gray-400">
                      💡 Click on any bus below to select it
                    </div>
                  </li>
                )}
                ...
                {vehicles
                  .filter(vehicle => 
                    ...
                    (vehicle.route?.name && vehicle.route.name.toLowerCase().includes(searchTerm.toLowerCase()))
                  )
                  .slice(0, 8) // Limit to 8 suggestions for better UX
                  .map((vehicle) => (
                    <li
                      key={vehicle.id}
                      onClick={() => selectBus(vehicle.registrationNo)}
                      className="px-4 py-3 hover:bg-blue-50 dark:hover:bg-blue-900/20 cursor-pointer border-b border-gray-100 dark:border-gray-700 last:border-b-0 transition-colors duration-150"
                    >
                      <div className="flex flex-col">
                        ...
                        <div className="flex gap-4 text-xs text-gray-500 dark:text-gray-400 mt-1">
                          ...
                          {vehicle.route?.name && (
                            <span>🛣️ {vehicle.route.name}</span>
                          )}
                        </div>
                      </div>
                    </li>
