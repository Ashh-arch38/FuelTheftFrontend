...
interface BusDetails {
  ...
  status: "normal" | "offline" | "faulty" | "warning";
    ...
    useEffect(() => {
      ...
      if (vehicle) {
        setBusDetails({
          ...
          status: "normal" as "normal" | "offline" | "faulty" | "warning", // Will be updated when data is fetched
        });
      }
  ...
  useEffect(() => {
    ...
    const fetchBusData = async () => {
      try {
        ...
        // 1) Fetch sensor status from the correct sensor API endpoint
        let sensorStatus = "offline";
        let allowedSensorIds: string[] = [];
        let lastSeen = null;
        try {
          const sensorRes = await axios.get<any>(`${API_BASE_URL}/sensor?busId=${selectedVehicleId}`);
          ...
          // Fallback to old sensor endpoint (this should now work since we're using the correct path)
          try {
            ...
            if (sensors.length === 0) {
              ...
              const allActive = sensors.every((s: any) => s.isActive !== false);
              const hasWarnings = sensors.some((s: any) => s.isActive === "warning" || s.status === "warning");
              const hasAlerts = sensors.some((s: any) => s.isActive === "alert" || s.status === "alert" || s.status === "alarm");
              const hasErrors = sensors.some((s: any) => s.isActive === false || s.status === "error" || s.status === "faulty");
        ...
        setBusDetails(prev => ({
          ...prev!,
          currentFuelLevel,
          status: sensorStatus as "normal" | "offline" | "faulty" | "warning",
          lastSeen,
        }));

        // Try to fetch fuel usage stats
        let finalStats: FuelUsageStats;
        let backendRefueled = 0;
        let backendStolen = 0;
        ...
        try {
          ...
          // Use backend stats as primary source
          finalStats = { ...usage.data, fuelEfficiency: usage.data.fuelEfficiency ?? 0 };
          backendRefueled = usage.data.totalFuelRefueled || 0;
          backendStolen = usage.data.totalFuelStolen || 0;
          ...
          try {
            ...
            if (summaryData && !summaryData.message) {
              finalStats = {
                ...
                totalFuelRefueled: summaryData.totalFuelRefueled ?? 0,
                totalFuelStolen: summaryData.totalFuelStolen ?? 0,
                distanceTravelled: summaryData.distanceTravelled ?? 0,
                fuelEfficiency: summaryData.fuelEfficiency ?? 0,
              };
              backendRefueled = summaryData.totalFuelRefueled || 0;
              backendStolen = summaryData.totalFuelStolen || 0;
              console.log("Using summarymatrix data:", summaryData);
            } else {
              // Initialize with alert-based fallback
              finalStats = {
                totalFuelConsumed: 0,
                totalFuelRefueled: 0,
                totalFuelStolen: 0,
